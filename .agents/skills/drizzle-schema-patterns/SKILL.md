...
|---|---|---|
| `system` | Auto-generated by DB/infrastructure. Visible to clients in responses, but never written explicitly. | `id`, `createdAt`, `updatedAt` |
...
| `createOnly` | Client may supply on creation; locked after that. | `type`, `productId` |
...
createTableSchemas(table, config) →
  ...
  clientCreate  minus system+serverOnly  POST request bodies
  clientUpdate  minus system+serverOnly+createOnly, all partial, id required  PATCH request bodies
```
